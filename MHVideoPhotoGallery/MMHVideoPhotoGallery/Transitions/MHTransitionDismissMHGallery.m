...
-(void)startInteractiveTransition:(id<UIViewControllerContextTransitioning>)transitionContext{
    ...
    if (imageViewerCurrent.isPlayingVideo && imageViewerCurrent.moviePlayer) {
        ...
        [self.moviePlayer.view setFrame:AVMakeRectWithAspectRatioInsideRect(imageViewerCurrent.moviePlayer.preferredContentSize,fromViewController.view.bounds)];
    ...
    if (self.toTransform != self.orientationTransformBeforeDismiss && !self.wrongTransform) {
        if (self.moviePlayer) {
            [self.moviePlayer.view setFrame:AVMakeRectWithAspectRatioInsideRect(imageViewerCurrent.moviePlayer.preferredContentSize,CGRectMake(0, 0, fromViewController.view.bounds.size.width, fromViewController.view.bounds.size.height))];
